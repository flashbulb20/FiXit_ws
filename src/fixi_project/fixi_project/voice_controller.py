...
class VisionPointingVoiceNode(Node):
    def __init__(self):
        ...
        2. 작업도구에는 pcb, 플럭스(flux), 전선, 돋보기(magnifier), 납 흡입기(pump) 가 있습니다.
        3. 직접적으로 도구를 언급하지 않아도 문맥을 파악하여 필요한 도구를 알아서 선택하세요. 예)부품을 교체해야 함 -> 흡입기, 납이 잘 안녹음 -> flux
        ...
        6. 작업 보조:
            ...
            - "자동모드" -> auto
            - "수동모드" -> manual
        7. 물체 이름 매핑: 사용자가 부르는 용어가 달라도 표준 영어 단어를 사용하여 'fetch_단어' 형태로 만드세요. 단 2번에 괄호가 붙어있는 단어는 그 괄호의 영어로 출력하세요.
